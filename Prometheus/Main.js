...
const pattern = /([A-Za-z]\[[A-Za-z]\]) = [A-Za-z]\([A-Za-z]\)/g; // Regular expression to match table assignments (e.g., a[b] = c)
...
file_system.readFile('input.lua', 'utf8', (read_error, script) => { // Read the script file
  ...
  while ((regex_match = pattern.exec(beautified)) !== null) { // Loop through each match of the pattern
    ...
    const matched_variable = regex_match[1]; // Extract the variable name from the match for the print statement
